  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 transition-colors duration-300">
      ...
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-zinc-200/60 dark:border-zinc-800/60">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            <div className="w-9 h-9 bg-gradient-to-br from-zinc-900 to-zinc-700 dark:from-white dark:to-zinc-300 rounded-lg flex items-center justify-center shadow-md group-hover:scale-105 transition-transform">
              <span className="text-white dark:text-zinc-900 font-bold text-sm">H</span>
            </div>
          ...
          <nav className="flex items-center gap-1">
            {['首页', '关于'].map((item) => (
              <Link
                key={item}
                href={item === '首页' ? '/' : `/${item}`}
                className="px-4 py-2 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all"
              >
                {item}
              </Link>
            ))}
      ...
      <section className="relative py-28 px-6 overflow-hidden">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[500px] bg-gradient-to-b from-blue-500/5 to-transparent dark:from-blue-500/10 rounded-full blur-3xl" />
        
        <div className="relative max-w-3xl mx-auto text-center">
          <p className="text-sm font-medium text-blue-600 dark:text-blue-400 mb-4 tracking-wider uppercase">
            技术博客
          </p>
          <h1 className="text-4xl md:text-6xl font-bold text-zinc-900 dark:text-white mb-6 leading-tight tracking-tight">
            记录学习
          ...
          <div className="flex items-center justify-center gap-3 text-sm text-zinc-500 dark:text-zinc-500">
      ...
      <main className="max-w-5xl mx-auto px-6 pb-24">
        <div className="flex items-center gap-3 mb-10">
          ...
          <h2 className="text-lg font-semibold text-zinc-900 dark:text-white">最新文章</h2>
        </div>
            ...
            >
              ...
              <div className="flex-1 p-5 flex flex-col">
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-2.5 py-1 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[11px] font-medium rounded-md">
                    {post.category}
                  </span>
                  <span className="text-xs text-zinc-400 dark:text-zinc-500">{post.date}</span>
                </div>
                <h3 className="font-semibold text-zinc-900 dark:text-white text-base leading-snug mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2">
                  {post.title}
                </h3>
