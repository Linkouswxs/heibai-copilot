...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f4ecd8] text-[#171717] pixel-grid">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-8 space-y-8">
        <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1.35fr_0.65fr]">
          <div className="pixel-card p-6 md:p-8 space-y-4 relative overflow-hidden">
            <div className="landing-ribbon">HOT</div>
            <p className="text-xs uppercase tracking-[0.3em] text-[#6d5a36]">像素开发日志 · 赛博日更</p>
            <h2 className="pixel-heading text-2xl leading-tight md:text-3xl">用像素风记录 AI 与工程成长</h2>
            <p className="text-sm md:text-base leading-7 text-[#2f2f2f]">
              从 Next.js 工程实践到 AI 协作工作流，这里是黑白搭档的公开实验场。每篇文章都可追溯、可复现、可讨论。
            </p>
            <div className="flex flex-wrap gap-3 pt-1">
              <Link href={posts[0] ? `/posts/${posts[0].id}` : '/'} className="pixel-btn pixel-btn-active">开始阅读</Link>
              <button onClick={() => setActiveCategory('技术')} className="pixel-btn">查看技术专栏</button>
            </div>
          </div>

          <div className="space-y-4">
            <div className="pixel-card p-4">
              <p className="text-xs text-[#6d5a36] mb-2">社区热度</p>
              <p className="pixel-heading text-lg">{posts.length} POSTS</p>
              <p className="text-sm mt-1">总点赞 {stat.likes} · 总评论 {stat.comments}</p>
            </div>
            <div className="pixel-card p-4">
              <p className="text-xs text-[#6d5a36] mb-2">本周推荐</p>
              <p className="text-sm leading-6">{posts[0]?.title ?? '内容加载中...'}</p>
            </div>
            <div className="pixel-card p-4">
              <p className="text-xs text-[#6d5a36] mb-2">订阅提示</p>
              <p className="text-sm">关注站点，第一时间接收新文章推送。</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {[
            ['工程化', 'SSR / SSG / API 一体化开发'],
            ['AI 协作', '从提示词到流程闭环优化'],
            ['实战复盘', '每篇文章都包含可执行经验'],
          ].map(([title, desc]) => (
            <article key={title} className="pixel-card p-4 md:p-5 landing-feature">
              <h3 className="pixel-heading text-base mb-3">{title}</h3>
              <p className="text-sm leading-6">{desc}</p>
            </article>
          ))}
        </section>

        <section className="pixel-card overflow-hidden p-4 md:p-5">
        ...
        {!loading && posts.length === 0 && (
          <div className="pixel-card p-8 text-center text-sm">暂无文章，切换一个分类试试。</div>
        )}

        <section className="pixel-card p-5 md:p-6 text-center">
          <h3 className="pixel-heading text-lg">READY TO BUILD WITH PIXELS?</h3>
          <p className="mt-3 text-sm leading-6">订阅黑白搭档博客，获取更多工程复盘、AI 实战与开发技巧。</p>
          <div className="mt-4 flex justify-center gap-3">
            <button className="pixel-btn pixel-btn-active">立即订阅</button>
            <button className="pixel-btn">查看归档</button>
          </div>
        </section>
      </main>
    </div>
